...
- ALL tools (incl. compress/update) require `projectRoot` + read config via `loadConfig(projectRoot)` — one path, no walk-up/guessing. Exception: startup guidanceLevel reads cwd (no projectRoot exists pre-ListTools).
...
- `guidanceLevel` (.aireadme.config.json, high default|medium): read SYNC from cwd at startup — ListTools ships before any call, so no projectRoot. high = old prompt verbatim. See src/core/verbosity.ts.
